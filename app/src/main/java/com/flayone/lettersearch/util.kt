...
data class LettersModel(
        var letter: String = "",//首字母
        var pinyin: String = "",//全拼音
        var name: String = "暂无"//名称
):Serializable

class LettersSorting : Comparator<LettersModel> {

    override fun compare(lettersModel: LettersModel, t1: LettersModel): Int {
        //给我两个对象，我只比较他的字母
        return lettersModel.letter.compareTo(t1.letter)
    }
}

/**
 * 通过首字母获取该首字母要显示的第一个的下标
 */
fun getFirstPosition(mList: List<LettersModel>, positionLetter: Char): Int {
    return (0 until mList.size).firstOrNull { mList[it].letter.toUpperCase()[0] == positionLetter }
            ?: -1
}
...
private fun getChsAscii(chs: String?): Int {
    ...
    try {
        ...
        if (bytes.size > 2 || bytes.isEmpty()) { // 错误
            // log
            throw RuntimeException("illegal resource string")
            // System.out.println("error");
        }
        if (bytes.size == 1) { // 英文字符
            asc = bytes[0].toInt()
        }
        if (bytes.size == 2) { // 中文字符
 ...
 * 转换一个或多个汉字
 */
fun convertAll(str: String): String {
    var result = ""
    var strTemp: String? = null
    for (j in 0 until str.length) {
        strTemp = str.substring(j, j + 1)
        val ascii = getChsAscii(strTemp)
        if (ascii in 1..159) {
            result += ascii.toChar().toString()
        } else {
            for (i in pyvalue.size - 1 downTo 0) {
